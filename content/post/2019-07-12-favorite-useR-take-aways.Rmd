...
<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">It was a pleasure and so random to translate the speach from the mayor of <a href="https://twitter.com/Toulouse?ref_src=twsrc%5Etfw">@Toulouse</a> at the <a href="https://twitter.com/hashtag/useR2019?src=hash&amp;ref_src=twsrc%5Etfw">#useR2019</a> welcome reception. <a href="https://t.co/YYLv68MXYM">pic.twitter.com/YYLv68MXYM</a></p>&mdash; Romain François 👨
👧
👧 (@romain_francois) <a href="https://twitter.com/romain_francois/status/1148641180332318721?ref_src=twsrc%5Etfw">July 9, 2019</a></blockquote>
...
FWIW: I am not the only one who felt the need express how special atmthe osphere was. [Robin Lovelace has also just posted his impressions from useR!2019](https://www.robinlovelace.net/2019/07/13/user2019/). And we got another thing in common: RobLike several others, Robin also stayed on the ground to get to Toulouse despite the fact that the southern France town is not exactly his nearest neighbor.

## 2. How to never forget that you *wanted* to fix these things *later* - the todor package

My other take aways are a bit more random as they mostly introduce packages. It's hard to single out packages or talks and my choice often depends on whether I knew these packages before.
...
- TEMP
 ...
 [todor](https://CRAN.R-project.org/package=todor) is a small but useful package to help you find such a NOTE in a file, package or folder:
...
The results are shown in R Studio's marker window.# 3. Are you teaching (or onboarding employees)? Check out datasciencebox.org

It's hard to summarize a good summary. So I'll leave you with what I just tweeted a while ago and hope it makes you read datasciencebox.org

<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">&#39;Best&#39; is a non-optimal label /w all the talks at <a href="https://twitter.com/hashtag/useR2019?src=hash&amp;ref_src=twsrc%5Etfw">#useR2019</a>, but <a href="https://twitter.com/minebocek?ref_src=twsrc%5Etfw">@minebocek</a> on <a href="https://t.co/NXUdWBL6VO">https://t.co/NXUdWBL6VO</a> was one of the best talks I attended. Ever. Made me feel to be on the right track, but still got many inspirations. Not only for teaching students but also for onboarding staff.</p>&mdash; Matt Bannert (@whatsgoodio) <a href="https://twitter.com/whatsgoodio/status/1148927130711011328?ref_src=twsrc%5Etfw">July 10, 2019</a></blockquote>
<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
...
<blockquote class="twitter-tweet" data-lang="en"><p lang="en" dir="ltr">Here&#39;s your feedback <a href="https://twitter.com/_ColinFay?ref_src=twsrc%5Etfw">@_ColinFay</a>: Loved it 🔥. Though I&#39;ve a fair share of R programming on the notch, I slept on pimping <a href="https://twitter.com/rstudio?ref_src=twsrc%5Etfw">@rstudio</a>. Well chosen examples: Doable and very instructive. Learned in minutes how create a wikipedia lookup addin. optimistic I&#39;ll remember it. <a href="https://twitter.com/hashtag/useR2019?src=hash&amp;ref_src=twsrc%5Etfw">#useR2019</a> <a href="https://t.co/Rxf49b45F8">pic.twitter.com/Rxf49b45F8</a></p>&mdash; Matt Bannert (@whatsgoodio) <a href="https://twitter.com/whatsgoodio/status/1148533934776639489?ref_src=twsrc%5Etfw">July 9, 2019</a></blockquote>
<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>

Though customizing R Studio is definitely easy to do, a good kickstart like Colin Fay's workshop always helps. 
Hereou couldn't attend it, I recommend to work through the examples in particular. Working through won't take long, but his examples are very instructive: 
...
Good news if you did not attend useR!2019, saw all the buzz on twitter and elsewhere
and felt like you missed something: There will another useR! [useR!2020 in St. Louis](https://twitter.com/user2020stl) is in the making.

Plus there is [eRum](https://twitter.com/erum2020_conf) which takes place in Europe whenever useR! is on another continent. 
So,if you live in the old world and prefer to travel by train, eRum seems to be preta ty cool option as well. It will take place in Milano in May 2020. 

And if you work in official statistics or anything close that,o itRos in Vienna](https://www.r-project.ro/conference2020.html) is for you. uRos is obviously more specof ial interest and gathering way smaller than the big R conferences but (just like Vienna itself) always worth your consideration. 


## 6. Feature-based time series analysis - the feasts package

If there is one name that's been synonymous to ftoecasting with R it's Rob J. Hyndman. Among other things, he and his team presented the idea of feature extraction from time series and how such an extraction can be implemented using the tidyverse. Features can be seen as properties of time series that allow for a comparison of time series. So, how *seasonal* is your series? How do certain series perform given their features? 
...
We can immediately see from the graph abht holidays are more seasonal than other travel, pl and thattern Australia (WA) has the strongest trends. 

More feature based examples can befoun d in the slides as wend[tidyverts.org](https://tidyverts.org). 
...
The colorspace project got you covered. Colorspace is the ultimate no-nonsense approach to colors and color palettes in R. The approach is comprehensive and very instructive. This does not come as a surprise: listthe  of names behind the package around maintainer Achim Zeileis reads like the Ocean's elevEn of the R Worlw. His talk gave interesting insights into how various three dimensional representation of colors, e.g., HCL vs RGB actually work, but also offered practical, application minded advice. 

What I liked best in that regard was the grouping of color palettes into **qualitative** color palettes that 
only change the type of color (hue), sequential single hue palettes, se that change the intensity of the same colorquential multi hue color palettes and diverging palettes. 

 that go into two directions, starting from the middle```{r}
library(colorspace)
...
![colorpicker](colo/img/rpicker.png)

The documentation and additional material are tremendous as well:
...
"Modern C++ routing engine for shortest path in road networks" That how project OSRM (Open Streetmap Routing Machine) describes itself on [their website](http://project-osrm.org/). And guess what there's an R package for it. So what do with it? 

Megan Beckett asked "What can I see from my AirBnB with my running shoes on" and lost no time 
answering the question in her lightning talk. For those of you who enjoyed the idea, but found the lightning talk a bit too lightning I've tried to adapt Megan's Toulouse example to my Zurich office. 
...
But before you start hacking, get your area from [Open Street Map](https://www.openstreetmap.org/) and use the export function. I suggest to rename the *map* file you receive to *map.xml* and put it in a separate folder.
...
This may take a bit depending on the size of your export, the profile (there are foot.lua and car.lua).
Once you're done simply fire up the OSRM server in order to use its API from R. 
